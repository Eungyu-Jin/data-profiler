...
class DataInfer:
    ...
    def __init__(self, data, columns=None, bins=10, smoothing=False, unit_of_time='s'):
        if isinstance(data, pd.DataFrame):
            self.tensor = Tensor(data.values, names=data.columns.to_list())
        elif isinstance(data, np.ndarray):
            self.tensor = Tensor(data, names=columns)
        else:
    ...
    def to_tensor(self, dtype):
        names = self.tensor.dindex(dtype=dtype)[1].tolist()
        if len(names) == 0:
            return None
        return Tensor(self.tensor.dslice(dtype), names = names)
    ...
    def infer_num(self)->NamedTuple:
        ...
        num_tensor = self.to_tensor('nums')

        return res(
            num_tensor.names,
            num_tensor.mean(axis=0),
            num_tensor.std(axis=0),
            num_tensor.min(axis=0),
            num_tensor.quantile(q=0.25, axis=0),
            num_tensor.quantile(q=0.5, axis=0),
            num_tensor.quantile(q=0.75, axis=0),
            num_tensor.max(axis=0),
            num_tensor.mode(axis=0),
            num_tensor.skew(axis=0),
            num_tensor.kurtosis(axis=0),
            num_tensor.bucket(self.bins, self.smoothing)
        ) if num_tensor != None else res()
    ...
    def infer_cat(self)->NamedTuple:
        ...
        cat_tensor = self.to_tensor('str')

        if cat_tensor != None:      
            lbs, cnt = cat_tensor.unique()
            return res(
                cat_tensor.names,
                lbs,
                cnt
    ...
    def infer_dt(self)->NamedTuple:
        ...
        dt_tensor = self.to_tensor('dt')

        return res(
            dt_tensor.names,
            dt_tensor.time_edge(False).astype(str),
            dt_tensor.time_edge(True).astype(str),
            dt_tensor.freq(self.unit_of_time),
            dt_tensor.cycle(self.unit_of_time)
        ) if dt_tensor != None else res()
...
class DataValidate():
    ...
    def __init__(self):
        self.jsd_threshold = 0.001
        self.lin_threshold = 0.001
    ...
    def jensen_shannon_divergence(self, base, eval, field):
        ...
        return jensenshannon(base_bucket_values, eval_bucket_values)
